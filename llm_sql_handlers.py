...
db_all = SQLDatabase.from_uri("sqlite:///med_db/doctor_tom.db",
                              include_tables=['clinical_trial', 'faq_chinese', 'doctor_spanish', 'doctor'])
...
db_ct = SQLDatabase.from_uri("sqlite:///med_db/clinicaltrials.db", include_tables=['doctor'],
                             max_string_length=3000)  # 2023.06.12 clinical_trial，默认300字
...
llm = OpenAI(model_name="text-davinci-003", temperature=0, verbose=True)  # davinci旧模型,4k
...
all_tables = ['doctor', 'clinical_trials_detail', 'pubmed_detail', 'cde_detail', 'medical_research', 'publications', 'personal_experience', 'achievements']
...
def get_table(user, table='ct', query=None, lang="简体中文"):
    ...
    if user:
        ...
        table_names = ['doctor', 'clinical_trials_detail', 'pubmed_detail', 'medical_research',
                       'publications',
                       'personal_experience', 'achievements']
        tables2 = {"all": db_user, 'doctor': db_user, 'clinical_trials_detail': db_user, 'pubmed_detail': db_user,
                   'medical_research': db_user, 'publications': db_user, 'personal_experience': db_user,
                   'achievements': db_user}
    if table == 'auto':
        # prompt = prompts.INTENT_TO_TABLE_PROMPTS
        prompt = prompts.INTENT_TO_TABLE_PROMPTS_V2
