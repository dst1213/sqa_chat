...
def get_pubmed_data(html):
    ...
    publications = soup.find_all('li')

    for publication in publications:
        data = {}

        # Extract PMID
        pmid_link = publication.find('a', href=lambda href: href and 'pubmed' in href)
        if pmid_link:
            pmid = pmid_link.text
            data['PMID'] = pmid
            data['URL'] = 'https:' + pmid_link['href']

        # Extract title
        title_div = publication.find('div', {'class': 'docsum-title'})
        if title_div:
